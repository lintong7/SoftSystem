...
public class LogServlet extends BaseServlet {
	...
	public String query(HttpServletRequest request, HttpServletResponse response) throws SQLException {
		...
		int count = (int) logDao.getLogCount(); // 日志总数
		long pageSize = 10L; // 每页多少条记录
		...
		List<Map<String, Object>> logs = logDao.getLogByPage(firstRow, pageSize); //分页查询
		...
		request.setAttribute("logs", logs); // 类似于map.put(key, value);直接目的数据放给JSP页面用
		return "f:/admin/logs.jsp"; // 指定响应给用户的页面地址
	}
}
